...
namespace nav2_lidar_localization
{

LidarLocalization::LidarLocalization(const rclcpp::NodeOptions & options)
: nav2_util::LifecycleNode("lidar_localization", "", options)
{
...
CallbackReturn LidarLocalization::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  odom_pose_with_cov_ptr_ = std::make_shared<geometry_msgs::msg::PoseWithCovariance>();

  RCLCPP_INFO(get_logger(), "Configuring end");
  return CallbackReturn::SUCCESS;
}

CallbackReturn LidarLocalization::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  if (use_pcd_map_) {
    pcl::PointCloud<PointType>::Ptr map_cloud_ptr(new pcl::PointCloud<PointType>);
...
CallbackReturn LidarLocalization::on_deactivate(const rclcpp_lifecycle::State &)
{
...
CallbackReturn LidarLocalization::on_cleanup(const rclcpp_lifecycle::State &)
{
...
CallbackReturn LidarLocalization::on_shutdown(const rclcpp_lifecycle::State & state)
{
...
CallbackReturn LidarLocalization::on_error(const rclcpp_lifecycle::State & state)
{
...
void LidarLocalization::initializeParameters()
{
...
void LidarLocalization::initializePubSub()
{
  ...
  initial_pose_sub_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
    ...
    std::bind(&LidarLocalization::initialPoseReceived, this, std::placeholders::_1));

  map_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
    "map", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable(),
    std::bind(&LidarLocalization::mapReceived, this, std::placeholders::_1));

  odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
    "odom", rclcpp::SensorDataQoS(),
    std::bind(&LidarLocalization::odomReceived, this, std::placeholders::_1));

  cloud_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
    "cloud", rclcpp::SensorDataQoS(),
    std::bind(&LidarLocalization::cloudReceived, this, std::placeholders::_1));

  imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
    "imu", rclcpp::SensorDataQoS(),
    std::bind(&LidarLocalization::imuReceived, this, std::placeholders::_1));

  RCLCPP_INFO(get_logger(), "initializePubSub end");
}

void LidarLocalization::initializeRegistration()
{
  ...
  if (registration_method_ == "GICP") {
    ...
    boost::shared_ptr<pcl::NormalDistributionsTransform<PointType, PointType>> ndt(
    ...
    pclomp::GeneralizedIterativeClosestPoint<PointType, PointType>::Ptr gicp_omp(
...
void LidarLocalization::initialPoseReceived(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
...
void LidarLocalization::mapReceived(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
...
void LidarLocalization::odomReceived(const nav_msgs::msg::Odometry::ConstSharedPtr msg)
{
  odom_pose_with_cov_ptr_->pose = msg->pose.pose;
...
void LidarLocalization::imuReceived(const sensor_msgs::msg::Imu::ConstSharedPtr msg)
{
...
void LidarLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  last_scan_ptr_ = msg;
  ...
  pcl::PointCloud<PointType>::Ptr tmp_ptr(new pcl::PointCloud<PointType>(tmp));
  ...
  Eigen::Affine3d odom;
  tf2::fromMsg(odom_pose_with_cov_ptr_->pose, odom);

  Eigen::Matrix4f odom_transformation = odom.matrix().cast<float>();

  pcl::PointCloud<PointType>::Ptr output_cloud(new pcl::PointCloud<PointType>);
  // rclcpp::Clock system_clock;
  // rclcpp::Time time_align_start = system_clock.now();
  rclcpp::Time time_align_start = this->get_clock()->now();
  registration_->align(*output_cloud, init_guess);
  // rclcpp::Time time_align_end = system_clock.now();
  rclcpp::Time time_align_end = this->get_clock()->now();
  ...
  Eigen::Matrix4f final_odom_transformation = final_transformation * odom_transformation.inverse();
  Eigen::Matrix3d odom_rot_mat = final_odom_transformation.block<3, 3>(0, 0).cast<double>();
  Eigen::Quaterniond odom_quat_eig(odom_rot_mat);
  geometry_msgs::msg::Quaternion odom_quat_msg = tf2::toMsg(odom_quat_eig);
  // transform_stamped.header.stamp = tf2_ros::toMsg(tf2_ros::fromMsg(msg->header.stamp) + tf2::durationFromSec(1)); // transform tolerance
  transform_stamped.header.stamp = msg->header.stamp;
  transform_stamped.header.frame_id = global_frame_id_;
  transform_stamped.child_frame_id = odom_frame_id_;
  transform_stamped.transform.translation.x = static_cast<double>(final_odom_transformation(0, 3));
  transform_stamped.transform.translation.y = static_cast<double>(final_odom_transformation(1, 3));
  transform_stamped.transform.translation.z = static_cast<double>(final_odom_transformation(2, 3));
  transform_stamped.transform.rotation = odom_quat_msg;
...
RCLCPP_COMPONENTS_REGISTER_NODE(nav2_lidar_localization::LidarLocalization)
